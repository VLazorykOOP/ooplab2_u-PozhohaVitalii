...
void task1() {
	...
	cout << "Expression : x = (127 * a + 32*c)/4096-d * 1200 + b * 131 \n";
	cout << " Input a  b  c  d : ";
...
void task3()
{
		...
		for (int i = 0; i < 8; i++) {
			for (int j = 0; j < 8; j++) {
				...
				res[i][j].bitpSecondTwoFields = pbit1(res[i][j].numberPosRow) ^ pbit1(res[i][j].molodshaChastyna);

				res[i][j].starshaChastyna = a[i][j] >> 4;
				res[i][j].posRow = j;

				res[i][j].bitpLastTwoFields = pbit1(res[i][j].starshaChastyna) ^ pbit1(res[i][j].posRow);
